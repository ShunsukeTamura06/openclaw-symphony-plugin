  ...
  base: {
    ...
    agentPrompt: {
      messageToolHints: () => [
        ...
        "Symphony renders MessageML (XML), not Markdown. The channel auto-converts these Markdown",
        "elements to MessageML before sending:",
        "  - headings #..######, **bold**, *italic*, `inline code`",
        "  - fenced code blocks ```lang ... ``` → <code language=\"lang\"> with syntax highlighting",
        "    (recognized langs: c, cpp, csharp, css, html, java, js, jsx, json, markdown,",
        "    php, plaintext, python, r, scala, shell, tsx, typescript, yaml — unrecognized",
        "    languages fall back to plaintext)",
        "  - ordered/unordered/nested lists",
        "  - hyperlinks [label](url) — only http(s)/mailto/tel schemes are kept",
        "  - horizontal rules ---",
        "  - GFM tables",
        "",
        "**NOT supported by Symphony** (these degrade to plain text — avoid relying on them):",
        "  - blockquote (>) — the > marker is dropped, content kept",
        "  - strikethrough (~~text~~) — emitted as plain text",
        "  - underline — no MessageML tag exists",
        "  - inline images — only the alt text is kept",
        "",
        "**Symphony-only features** (write these as MessageML directly — no Markdown equivalent):",
        "- Mention a user: `<mention email=\"alice@example.com\"/>` or `<mention uid=\"12345\"/>`",
        "- Emoji shortcode: `<emoji shortcode=\"smile\"/>`",
        "- Hashtag: `<hash tag=\"important\"/>`",
